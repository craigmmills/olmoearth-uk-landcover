...
class TestLoadIterationDetails:
    ...
    def test_baseline_has_no_hypothesis(self, tmp_path):
        ...
        experiments_dir = tmp_path / "experiments"
        _create_iteration(experiments_dir, 1)
    ...
    def test_reads_hypothesis_from_previous_iteration(self, tmp_path):
        ...
        _create_iteration(experiments_dir, 1, hypothesis=hypothesis)
        _create_iteration(experiments_dir, 2)
    ...
    def test_loads_evaluation_data(self, tmp_path):
        ...
        _create_iteration(experiments_dir, 1, evaluation_2021=eval_data)
    ...
    def test_metrics_diff_for_second_iteration(self, tmp_path):
        ...
        experiments_dir = tmp_path / "experiments"
        _create_iteration(experiments_dir, 1, accuracy=0.70)
        _create_iteration(experiments_dir, 2, accuracy=0.75)
...
class TestRenderingDataContracts:
    """Verify data shapes that render_iteration_card depends on."""

    def test_hypothesis_missing_for_legacy_iteration(self, tmp_path):
        """Iteration > 1 without hypothesis.json in N-1 should have hypothesis=None."""
        from src.app import load_iteration_details

        experiments_dir = tmp_path / "experiments"
        # Create iteration 1 with NO hypothesis.json
        _create_iteration(experiments_dir, 1)
        # Create iteration 2 (should look for hypothesis in iteration_001)
        _create_iteration(experiments_dir, 2)
    ...
    def test_config_diff_keys_have_a_and_b(self, tmp_path):
        """Config diff entries should have 'a' (old) and 'b' (new) keys."""
        from src.app import load_iteration_details

        experiments_dir = tmp_path / "experiments"
        # Iteration 1: default config (n_estimators=100)
        _create_iteration(experiments_dir, 1)
        # Iteration 2: different config (n_estimators=200)
        _create_iteration(
            experiments_dir, 2,
            config_overrides={"training": {"n_estimators": 200}},
        )

        with patch("src.config.EXPERIMENTS_DIR", experiments_dir):
            details = load_iteration_details(2)

        assert details["config_diff"] is not None
        for param, vals in details["config_diff"].items():
            assert "a" in vals, f"Missing 'a' key in config_diff[{param!r}]"
            assert "b" in vals, f"Missing 'b' key in config_diff[{param!r}]"

    def test_evaluation_has_recommendations_list(self, tmp_path):
        """Evaluation data should contain a recommendations list."""
        from src.app import load_iteration_details
        ...
        eval_data = {
            ...
            "evaluation": {
                ...
                "confidence": 0.8,
                "recommendations": ["Improve cropland detection", "Add NDVI features"],
            },
            "summary": {"overall_score": 7, "confidence": 0.8,
                        "num_error_regions": 0, "num_recommendations": 2},
        }
        _create_iteration(experiments_dir, 1, evaluation_2021=eval_data)
        ...
        assert details["evaluations"]["2021"] is not None
        recs = details["evaluations"]["2021"]["evaluation"]["recommendations"]
        assert isinstance(recs, list)
        assert len(recs) == 2

    def test_evaluation_missing_returns_none(self, tmp_path):
        """Missing evaluation files should result in None values."""
        from src.app import load_iteration_details
